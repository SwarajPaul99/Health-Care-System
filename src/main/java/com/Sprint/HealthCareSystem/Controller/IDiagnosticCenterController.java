...
public class IDiagnosticCenterController {
	...
	@GetMapping("/view/{id}")
	public ResponseEntity<DiagnosticCenter> getDiagnosticCenterById(@PathVariable int diagcentreId) throws DataNotFound {
		DiagnosticCenter dc = diagnosticCenterService.getDiagnosticCenterById(diagcentreId);
		System.out.println(System.currentTimeMillis());
		return new ResponseEntity<DiagnosticCenter>(dc, HttpStatus.CREATED);
	}
